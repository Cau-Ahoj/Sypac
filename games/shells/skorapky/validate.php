...
try {
    ...
    if (!in_array($game_type, ['normal', 'double', 'collect'])) {
        throw new Exception("Neplatný typ hry.");
    }

    if ($game_type === 'normal') {
        // Odečti sázku pouze při startu (tj. když bet_amount > 0)
        if ($bet_amount > 0) {
        ...
        if ($is_win) {
            ...
            $db->log($user_id, 'game_loss', "Normální prohra.");
        }
    }
    ...
    elseif ($game_type === 'collect') {
        if ($won_amount > 0) {
            $result = $db->run("UPDATE users SET money = money + ? WHERE id = ?", [
                $won_amount, $user_id
            ]);
            if (!$result) {
                throw new Exception("Nepodařilo se přičíst výhru při sbírání.");
            }
            $db->log($user_id, 'collect_win', "Sbírání výhry: +{$won_amount}");
        }
    }

    // Záznam do historie
    $sql = "INSERT INTO shells_game_history (id_user, bet_amount, is_win, won_amount, played_at, game_type)
            VALUES (?, ?, ?, ?, NOW(), ?)";
    $db->run($sql, [
        $user_id,
        $bet_amount,
