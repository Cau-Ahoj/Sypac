...
if ($_SESSION["csrf_token"] == $_POST["csrf_token"]) {
    if (isset($_POST["username"], $_POST["password"], $_POST["password_confirm"], $_POST["email"], $_POST["phone"])) {
        ...
        $existing = $db->getOne("SELECT * FROM users WHERE username = ? OR email = ?", [$username, $email]);
        if ($existing) {
        ...
        if ($success) {
            ...
            header("Location: ./?error=db&username=" . urlencode($username) . "&email=" . urlencode($email) . "&phone=" . urlencode($phone));
            exit();
        }
